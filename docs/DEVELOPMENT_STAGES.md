...
| 当前阶段 | M1-Gate 已通过，M1 正式完成；当前允许开始 M2-0 ClawBot 技术 Spike，尚未开始 |
...
- 首发计划城市：深圳；收藏库允许记录其他城市的地点与活动。
- 当前 M0–M3 路线保持深圳单城市计划，不实现运行时计划城市切换；该能力作为后续独立升级点。
...
13. 收藏城市不是准入门槛；其他城市收藏可以保留，但只有正式城市与本次 Plan 城市一致的条目才可参与计划。
...
- Repository 接口；
- 用户隔离。
- `User` 默认计划城市与 `CollectionItem.city_hint` 分离；`city_hint` 可空且不代表正式城市；
- 已合并的深圳专用 `CollectionItem.city` 约束通过新的向前迁移调整，不改写已发布 `0003`。
...
- 完整合法日期范围或完整合法精确场次任一足以确认 Event 时间；日期范围仅在计划日期
  与范围相交时进入候选，没有精确场次时在用户空闲窗口内复用既有 visit duration
  排程；部分字段、无效范围、时间冲突或相关 uncertainty 仍保持待确认。
...
- POST Undo。
- GET Collection 支持城市、城市待确认、区域、类型、状态和标签筛选；只实现 API 契约，不提前开发前端。
...
- 明确深圳地点；
- 明确广州/上海等其他城市的单个地点和活动可形成候选；
- 一条输入包含不同城市的多个收藏对象；
- 只有店名；
- 店名或活动标题含“深圳/上海/广州”等城市词但城市仍待确认；
- 过于通用的名称；
- Event 文本；
- 不支持的菜谱或跨城多日规划需求；
- 空输入；
...
- 定义内部 POI DTO；
- 定义显式接收城市范围的 search_poi、get_poi、route、weather 和 URI 接口；
- 实现唯一结果、多个结果、无结果、超时 Fixture；
- Stub 至少覆盖深圳和一个其他城市，并证明调用间城市范围不污染；
- 领域层不得使用高德原始字段名。

##### M0-3B：真实高德适配

- 只在服务端使用 API Key；
- 每次查询显式传入城市范围；无城市线索时当前默认搜索深圳，但不得把默认搜索范围写成已确认城市；
- 记录 provider、poi_id、正式 city_code 和 GCJ-02；
- 设置超时、有限重试和响应校验；
- 真实集成测试显式开启。
...
- city（当前固定 `shenzhen`，但必须作为显式硬约束传递）；
- start_at、end_at；
...
- 正式城市与 Plan 城市一致；不得使用 `city_hint` 代替正式城市；
- 行政区；
- Place/Event；
- 收藏状态；
- 时间；
...
- 其他城市和城市待确认收藏保留在收藏库，但必须从当前深圳计划候选中排除并给出原因；
...
- 初次生成一个主方案和两个备选；
- 模型决定候选组合、顺序和建议停留时长；
- 主方案最符合用户原始要求和节奏，两个备选不预设固定风格；
- 不固定每项一小时，不限制每个方案只包含两个地点；
- 交通缓冲 10–20 分钟；
- 结束留白 15–30 分钟；
- 时间、路线、费用、来源和风险；
- 选择理由和排除原因；
...
M0 关闭后的 Event 日期粒度与富输入截止校准已通过主控验收，不重新打开 M0。
固定图片样本 03 在 60 秒应用截止和 75 秒 Provider 异常安全上限下约 47.1 秒成功，
正确保留日期范围且未编造准确时刻；样本 06 仍不作为内容正确性 Gate。校准没有增加
标题、截图或内容白名单等生产特例。当前允许开始 M1-0，其第一项强制前置仍是
...
13. 完成真实延迟基准与超时校准，并用测量数据说明当前模型、Agent、高德、网页和图片时限是否足够。

#### 真实延迟基准与超时校准

- 分别测量单次文本抽取、一次结构修复、模型 → Tool → 模型、高德查询、普通/重定向网页和图片识别，不用一个总体平均值替代不同链路；
- 同时记录单次外部调用和用户端到端流程的 P50、P95、最大耗时、超时率、重试率与失败恢复结果；复合流程必须验证外层总时限，不得只证明每个子调用单独未超时；
- 记录每类场景的样本数、冷/热启动条件、网络与部署区域。样本不足时只能报告观测值，不得宣称 P95 已得到统计验证；
- 普通回归继续使用 Fake、Mock、固定 Fixture 和封网测试。真实模型、高德、网页或图片取样必须在 M0-Gate 当前任务中逐项获得用户明确授权，并预先约定最大请求数和费用预算；
- 以正常 P95 仅占对应硬超时约 60%–75% 作为建议余量；接近或频繁触发上限时，优先缩短链路、增加外层总预算约束或转后台任务，不得只为消除失败而无限提高超时；
- 明确核对模型 Provider/传输层 75 秒异常安全上限不会先于应用层截止触发，高德多次尝试具有可解释的逻辑总时限；URL/图片使用一个 60 秒整链路共享硬兜底并作为正常路径唯一可触达的硬截止，initial 与唯一 repair 共用截止；20 秒只作为图片和网页解析的非阻断性能观察目标；真实同步请求仍超过 60 秒时转 M1 后台 Job，不继续提高上限；
- 根据结果更新配置示例、自动化边界测试和恢复策略；若决定不调整某项时限，必须在报告中记录测量依据和剩余风险。
...
- 通过与失败；
- 真实 API 验证范围；
- 各链路样本数、P50、P95、最大耗时、超时率、重试率与恢复结果；
- 调整前后的超时配置、端到端预算、Token 和费用；
- 未解决风险；
- 是否允许进入 M1。
...
**2026-08-01 最终结论：M1-Gate 已通过，M1 正式完成。** 真实运行复验发现的截图
导入、收藏补充与地点确认、Place/Event 和日期语义、截图与文字组合输入、Retry、
计划入口、移动端详情、SSE 权威终态、运行配置及图片截止测试稳定性问题均已完成
修复和独立主控复验。最终候选在全新快照中通过三轮后端非真实全集、封网全集、
PostgreSQL 16、迁移往返、前端单测/构建/E2E 及隔离 Compose 完整启动；当前没有
未关闭 P0/P1。完整证据见 `docs/technical/M1_VALIDATION_REPORT.md`。当前允许开始
M2-0 ClawBot 技术 Spike，但 M2-0 尚未开始，不能据此宣称微信接入已实现。
...
- 上述 20 秒是性能观察目标，不是同步流程强制取消阈值；URL/图片完整流程继续使用
  60 秒共享硬兜底。
- 任务重试不产生重复收藏、计划或文件；
- Demo 与真实数据隔离。
...
### 13.6 后续升级 U1：单活动计划城市切换（不进入当前排期）

#### 启动条件

- 跨城市收藏和深圳单城市计划核心闭环已经通过 M1-Gate，或用户另行明确调整优先级；
...
- 在现有跨城市收藏基础上，区分用户默认计划城市、Session 当前计划城市、收藏正式城市和 Plan 城市；
- 提供显式城市列表与 Session 城市切换能力，不使用进程级全局城市状态；
- 让 MapProvider、收藏检索和 PlanConstraints 显式接收当前计划城市；抽取继续允许任意城市收藏，不因 Session 城市拒绝；
...
- 其他城市的收藏继续保留，只有正式城市与当前 Plan 城市一致的收藏进入计划；
...
- 自动化测试验证公开行为、产品硬约束和真实使用入口，不以覆盖第三方库全部内部调用方式为目标；
- 不得为单个失败样例持续增加特例、字段/消息白名单、内部代理、重复 DTO 或平行校验器；
- 同一缺陷修复后出现第二个旁路时，开发窗口必须停止追加补丁，回到公开入口、信任边界和模块归属重新设计；
- 安全要求应在实际跨越信任边界的位置集中执行，例如不可信输入解析、Application/API 错误映射和日志出口，不侵入或替换第三方框架内部状态来追求“所有内部入口天然安全”；
- 收敛修复必须删除被替代的旧保护层，并证明业务校验仍只有一份；不能以保留旧实现作为“保险”为由形成多套路径；
- 阶段验收除功能测试外必须审查改动的净复杂度：若新增机制明显多于产品行为，或解释只能表述为“为了过测试”，则不得合并；
- 必要的 PRD 业务规则不属于堆规则，但必须可追溯到产品文档、集中归属并具有稳定语义。
