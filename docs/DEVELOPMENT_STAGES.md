...
- 首发城市：深圳。
- 当前 M0–M3 路线保持深圳单城市，不实现运行时城市切换；该能力作为后续独立升级点。
...
### 13.6 后续升级 U1：单活动城市切换（不进入当前排期）

#### 启动条件

- 深圳单城市核心闭环已经通过 M1-Gate，或用户另行明确调整优先级；
- 新增启用城市、地图覆盖和测试样本已经确认；
- 已通过 ADR 确认城市目录、会话切换、历史数据和迁移策略。

#### 任务

- 建立唯一 `CityCatalog`，管理稳定 `city_code`、展示名、时区、高德 `adcode`、行政区别名和启用状态；
- 区分用户默认城市、Session 当前城市、收藏实际城市和 Plan 城市；
- 提供显式城市列表与 Session 城市切换能力，不使用进程级全局城市状态；
- 让抽取、MapProvider、收藏检索和 PlanConstraints 显式接收城市范围；
- 对内容城市与当前城市冲突提供切换、保留当前范围或取消三种恢复路径；
- 为现有深圳数据提供可回滚迁移，并为每个启用城市建立独立 Fixture 与高德契约测试。

#### 验收

- 两个并发 Session 可以使用不同城市且互不污染；
- 切换城市不改变长期默认城市，除非用户另行明确设置；
- 其他城市的收藏不会被删除，但不会进入当前城市计划；
- 地点或活动标题中的城市词不会触发隐式切换；
- Provider、缓存、Repository、幂等和去重均按稳定城市范围工作；
- AgentRunner、ToolRegistry、ModelProvider 和业务工作流保持唯一。

#### 不做

- 不在一份计划中组合多个城市；
- 不实现跨城、多日旅行、机票或酒店；
- 不为每个城市复制 Provider、Repository、Prompt 或业务服务。
