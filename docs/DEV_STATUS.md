...
| 当前分支 | codex/m1-gate |
| 最近更新 | 2026-07-29 |
| 阻塞项 | P1：真实地图预检 5 组仅 2 组成功，待安全错误码定点诊断；M1-Gate 待主控最终验收 |
...
| M1-Gate 核心闭环验收 | 进行中 | 离线、封网、PostgreSQL 16、Compose、前端、21 组闭环通过；真实输入预检达目标但地图 5 组仅 2 组成功，尚未关闭 M1 |
...
地图 5 组仅 2 组成功，必须先按报告中的最小 Prompt 取得新的定点调用授权并记录
稳定安全错误码，区分外部环境与代码缺陷。阻塞关闭前 M1-Gate 不得标记完成。不得
提前实现微信、主动提醒、云部署或 M2。
