...
| 当前子阶段 | M1-4 收藏库与地点消歧 |
| 状态 | M1-4 待主控验收（收藏详情保存 P1 修复完成，待主控复验） |
| 当前分支 | codex/m1-4-collections-disambiguation |
| 最近更新 | 2026-07-28 |
| 阻塞项 | 无；M1-4 收藏详情保存 P1 修复完成，待主控复验，M1-5 不得提前开始 |
...
恢复与继续添加。M1-4 收藏库与地点消歧的 P1 修复已完成并等待主控复验；M1-5 及
后续阶段不得提前开发。
...
| M1-4 收藏库与地点消歧 | 待主控验收 | 收藏详情保存 P1 修复完成，待主控复验；原三个 P1 修复保持有效 |
...
主控从 `codex/m1-4-collections-disambiguation` 的独立修复提交复验 M1-4：
先以真实 ASGI/浏览器链路复验带标签收藏的详情保存和清空，再检查详情写操作归属、
候选合并后的 URL/刷新一致性、exact/any-branch 全来源迁移与幂等/用户隔离，并
回归组合筛选、规划隔离、版本冲突、响应式与键盘闭环。验收前不合并、不推送，
不开始 M1-5 计划或后续业务。
