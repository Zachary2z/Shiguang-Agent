...
| 当前子阶段 | M1-Gate 稳定化修复 |
| 状态 | M1-Gate 修复 3 完成，等待主控复验；计划闭环修复仍未开始，M2-0 继续阻塞 |
| 当前分支 | codex/m1-gate-event-time-interaction |
| 最近更新 | 2026-07-30 |
| 阻塞项 | 计划闭环修复仍未开始；M1-Gate 继续打开；M2-0 未开始且阻塞 |

## 2026-07-30 M1-Gate 修复 3 Event 日期与具体时间交互

- M1-Gate 修复 3 完成，等待主控复验；计划闭环修复仍未开始，M2-0 继续阻塞。
  收藏详情继续保留活动有效开始/结束日期，原两个 `datetime-local` 已收敛为只编辑
  `HH:mm` 的 `time` 控件。已有 `event_start_at/event_end_at` 只按
  `Asia/Shanghai` 提取具体时间；对应日期字段为空时，从准确时间的上海自然日恢复
  日期草稿，不读取浏览器当前日期、当前时间或所在时区，也不因打开详情自动写库。
- “确认并保存”继续复用唯一收藏 PATCH。开始和结束分别按对应有效日期与具体时间
  组合为带 `+08:00` 的 ISO 8601；单日活动允许日期相同，多日活动使用各自日期。
  清空已有具体时间会显式提交 `null`；只确认日期仍可保存且准确时间保持 `null`，
  服务端继续返回不可规划和“具体时段待补充”状态。
- 前端只校验本次输入的组合完整性与时间顺序：结束日期不得早于开始日期；具体开始/
  结束时间各自要求对应日期；完整组合后的结束时刻必须晚于开始时刻。同日按
  `HH:mm`、跨日按完整组合时刻比较。只填写一个具体时间可保存，但成功反馈明确仍
  待补充；规划资格、地点阻塞和保存结果继续只读服务端响应。
- 409、422、超时、取消和网络错误均保留日期与具体时间草稿。Event 地点未确认时，
  即使准确时段已确认，仍展示服务端 `location_unconfirmed` 阻塞；没有复制地点、
  Event 状态机或规划资格规则。
- 前端验证：lint、typecheck、build 通过；Vitest `106 passed`；指定收藏
  Chromium E2E `13 passed`，Event 表单在 320、390、768、1024、1440px 均无
  横向溢出，原生标签与键盘顺序通过。后端最小契约回归 Ruff、strict mypy
  （140 个源文件）通过，指定三组测试 `52 passed`；Alembic 唯一 head 仍为
  `20260729_0017`。
- 本轮未修改后端、领域契约、Prompt、抽取规则、PATCH DTO、Repository、数据库或
  迁移；未新增 Event 专用 Client/服务或第二套时间转换路径，原有两个前端转换函数
  原位收敛为“ISO → 上海日期/时间”和“日期 + 时间 → `+08:00` ISO”。未读取
  `.env`，真实模型、地图、网页和付费 API 调用为 0；未合并、未推送，也未开始
  计划闭环或 M2。
