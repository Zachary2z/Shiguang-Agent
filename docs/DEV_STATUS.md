...
| 当前分支 | main |
| 最近更新 | 2026-07-29 |
| 阻塞项 | 无未关闭 P0/P1；保留 M1-5 高基数候选截断、前端开发依赖审计与跨浏览器覆盖等 P2/风险 |
...
| M1-Gate 核心闭环验收 | 已完成 | 主控复核完整闭环、离线/封网全集、高德候选隔离、时间稳定性及 7 次零重试真实地图复测通过；M1 正式关闭 |
...
从最终 `main` 创建 `codex/m2-0-clawbot-spike`，只执行 M2-0 技术 Spike：
验证微信文字和截图经过单一 Channel Adapter 转为既有 `InboundMessage`，复用
FastAPI、Nanobot、统一输入、存储和 `OutboundMessage` 边界，并验证身份/会话/
消息/trace 映射、重复消息、断线与发送失败恢复。Spike 不通过不得开始 M2-1；
不得新增第二套业务状态机，不得提前实现账号绑定、主动提醒、云部署或其他 M2
功能。微信真实凭证、设备、消息发送及外部发布仍需当次明确授权。
