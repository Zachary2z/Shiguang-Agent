...
- 最终测试数量：M0-4C/共享抽取/Provider 聚焦命令退出 0，`148 passed`；M0-4A 存储契约/集成退出 0，`71 passed`；core 退出 0，`118 passed`；`tests/integration/test_migrations.py` 退出 0，`21 passed`；正式非真实全集退出 0，`1223 passed / 2 deselected`；默认全集退出 0，`1223 passed / 2 skipped`。仓库外临时 pytest 插件封锁 socket connect/connect_ex/create_connection 与 DNS getaddrinfo 后，非真实全集再次退出 0，`1223 passed / 2 deselected`
