...
| 状态 | include 语义 P1 已关闭；继续验收计划调整与确认；M2-0 未开始 |
| 当前分支 | codex/m1-plan-include-semantics |
| 最近更新 | 2026-08-12 |
| 阻塞项 | 无未关闭 P0/P1；完成 PRD 核心计划主链路验收前不得开始 M2-0 |

## 2026-08-12 计划级 include 语义 P1 真实复验

- 原三条可执行收藏（展馆、公园、咖啡店）和原始自然语言计划请求聚焦真实复验通过；
  三条收藏全部进入唯一计划模型，完整 `include` 继续作为整份计划目标传入，没有再次
  出现候选被清空、外部补充审批或 `NO_EXECUTABLE_DRAFT`。该 P1 已关闭，当前无未关闭
  P0/P1。
- 两轮独立执行均生成草稿；模型原始方案形状为一个主方案和两个备选，每个方案均包含
  三条收藏。确定性时间与路线校验保留一个完整可执行方案，其余不可执行方案被正常
  淘汰；未放宽现有执行安全边界。
- 实际调用范围：2 次非流式模型请求，14 次只读高德请求（天气 2、路线 12），自动重试
  为 0；没有鉴权、限流、超时或供应商错误，没有输出或提交完整响应、密钥及临时 QA
  脚本。
- 下一步：集成本修复后继续完成计划调整、版本生成和方案确认的 PRD 核心计划主链路
  真实用户验收；M2-0 仍未开始。

## 2026-08-12 计划级 include 语义修复

- 生产修复完成，等待真实用户复验，不据此宣布 P1 关闭；M2-0 仍未开始。
  `PlanConstraints.include` 继续作为整份计划目标进入唯一 `PlanProposalService`，不再
  作为每条收藏或外部 POI 必须同时满足全部目标的硬过滤条件。
- 已从唯一结构化收藏检索和外部 POI 范围判断中删除错误的 include 合取排除，并删除
  已无生产用途的 `INCLUDE_NOT_MATCHED` reason code、文案和失效断言。`exclude`、城市、
  行政区、Event、时间、预算、路线、可用性、selected、required、collection_only 和
  任意分店语义保持不变；没有新增规则规划器、关键词映射、fallback、重试或抽象。
- 离线回归使用 Fake、Stub 和固定 Fixture，覆盖三条收藏分别承担“看展、逛公园、
  喝咖啡”并全部进入一次模型提案、完整 include 传递、外部 POI 不受逐候选 include
  合取过滤，以及既有硬约束和外部审批恢复流程。真实 API 调用为 0，未读取 `.env`。
- 下一步：使用原始三收藏和原始自然语言请求进行聚焦真实复验；本修复不自行执行真实
  模型或高德调用。P1 只有在该复验通过后才可关闭，M2-0 继续保持未开始。
...
- `codex/m1-model-driven-planning` 已从 `ff24176722fefab4d2dae0bb0280e040be04767d`
  线性开发至 `78836525690cfa443f4c41a8652b3100b9bb24b6`，并以 `--ff-only`
  纯快进集成到 `main`；没有冲突、merge commit、历史改写或额外代码变化。
- 主控复验确认多轮计划追问只延续权威 `waiting_user` Job 上下文；成功的收藏、任意
  分店和 Memory 不再污染新计划。两个上下文 P1 均关闭，当前无未关闭 P0/P1。
- 候选提交聚焦回归 `142 passed, 1 skipped`，非真实 Provider/Map 全集
  `1776 passed, 18 skipped, 2 deselected`；合并后 pip check、Ruff、strict mypy
  （143 个源文件）、计划聚焦 `61 passed` 及 Alembic 唯一 head
  `20260729_0017` 全部通过。
- 本次集成未读取 `.env`，真实模型、地图、网页及其他外部 API 调用为 0；未推送远端。
  当前允许开始第三批“方案调整、单线版本、方案确认和 Web/H5 交互”，M2-0 仍未开始。
...
- 主控验收 P1 已按共同边界修复，当前状态为“第二批修复完成，等待主控复验”，不据此
  宣布验收通过。Agent 原始计划原句作为可空私有字段保存在既有 constraints JSON，
  幂等重放、外部地点授权恢复和 Worker 重启均读取同一值；公开 constraints、日志、
  分享和安全异常不输出原句。收藏表单没有原句时只生成已有结构化约束的事实摘要。
- 结构化检索把既有收藏标签传给脱敏模型候选；准确收藏与任意分店解析到同一 POI 时，
  唯一 Resolver 按全部收藏来源稳定复用同一具体 POI，并由一个 PlanItem 保存全部来源。
- 准确 Place/Event 在模型提案前只做静态硬约束过滤，不预取起点路线；生产 Resolver
  复用唯一 `proposal_route_edges`，仅查询三个实际方案使用的去重起点边和相邻边。
  任意分店解析所需路线继续复用；路线失败只淘汰引用该边的方案。
- 一个外部候选现在保存各方案实际入站边，草案按前序收藏来源选路，已删除首次出现位置
  路径；无收藏但已授权且外部 Place 提案有效时可生成草案，无精确起点的首段保持未知。

- 唯一 `PlanProposalService` 已接入现有 `ExistingPlanServicesExecutor` 和 Worker：硬过滤后
  由模型生成一个主方案和两个备选，唯一 `PlanDraftService` 按模型地点顺序和停留时长
  计算准确时间、交通缓冲并复核硬约束；单个非法提案直接丢弃，不替换地点。
- 唯一 `MapPlanFactResolver` 不再截断六个候选或预取候选全连接路线，只保留全部通过硬
  约束的候选，并在模型提案后获取实际使用的起点/相邻路线；无精确起点的首段距离和
  时长保持未知。
- 唯一 `ExternalPlaceSupplementService` 只处理明确 Place 缺口的授权、一次只读高德搜索
  和候选/恢复结果，不再生成草案。收藏核心存在时可自动补充一次；无收藏核心时先授权；
  `collection_only` 与 Event 缺口不外搜。成功补充后仅由同一模型服务再提案一次，外部
  地点继续标记“高德补充 · 未收藏”，没有收藏写入。
- 已删除固定六候选、固定一小时、固定稳定排序、一个核心加一个辅助、最多两个地点和
  候选全连接路线旧路径；没有规则方案 fallback、第二个 Resolver/Scheduler/Validator、
  新 Job、Worker、Repository、Provider、迁移、依赖或自动重试。
- 聚焦集合 `151 passed`；非真实 Provider/Map 全集 `1772 passed, 18 skipped, 2
  deselected`；`pip check`、Ruff、strict mypy（143 个源文件）、Alembic 唯一 head
  `20260729_0017` 和 `git diff --check` 通过。未读取 `.env`，真实模型、高德、网页及
  其他外部 API 调用为 0。第三批计划调整和方案确认未开始，M2-0 未开始。

## 2026-08-11 模型驱动计划生成第一批：产品契约与模型提案边界

- 当前批次为“产品契约与模型提案边界”；只建立唯一模型提案边界，不接入排程、Worker、API、前端、高德真实调用或计划调整。M2-0 未开始。

- 用户确认 Agent 自然语言仍是计划主入口，收藏多选是辅助入口；两者必须复用一条计划
  主链路。初次由唯一 `ModelProvider` 生成一个主方案和两个备选，主方案最符合原始要求
  和节奏，候选组合、顺序和停留时长不再由固定规则决定。
- 现有唯一 `PlanDraftService` 继续负责准确时间、路线和硬约束；现有
  `ExternalPlaceSupplementService` 继续作为唯一高德补充边界。不得增加第二套规划器、
  候选评分器、调整解析器或推荐服务。
- 任意方案均可作为调整和确认基础；调整只生成该方案的一个新版本，未涉及内容默认
  保留。确认方案序号复用现有 `draft_json`，执行、日历、导航、分享和反馈统一读取。
- 缺失精确起点时允许先生成草稿但不伪造首段路线；截图只出现月日时不得猜测年份。
- 设计见 `docs/technical/M1_MODEL_DRIVEN_PLANNING_DESIGN.md`，分步实施见
  `docs/superpowers/plans/2026-08-11-model-driven-planning.md`；缺失年份使用独立提交计划
  `docs/superpowers/plans/2026-08-11-yearless-event-confirmation.md`。
- 本轮只修改文档，未修改生产代码、依赖、数据库或迁移，真实 API 调用为 0。

## 2026-08-10 所选收藏计划可执行性 P1 修复

- 两个 P1 的共同根因已收敛在唯一 `assess_collection_candidate` 规划资格判断边界：
  `area.labels` 原本对自动候选和 `selected_collection_item_ids` 明确选择一律作为
  `AREA_MISMATCH` 硬拒绝，任意分店还会在搜索前的资格探测中被同一规则挡住。
- 原规则现在只对未明确选择的自动候选保持硬筛选；明确选择仍完整执行城市、行政区、
  状态、地点确认、Event 时间、显式排除、预算、路线、可用性和时间窗口规则。任意分店
  因而复用现有 `MapPlanFactResolver`、`PlaceMatchingService` 和路线稳定排序解析具体
  POI，计划快照继续保存 `concrete_poi`、`any_branch_collection_item_ids` 与
  `poi_queried_at`。普通准确地点确认策略和全局 75 分/12 分差未修改。
- 离线回归覆盖福田区/市民中心下两个明确选择的准确地点，以及南山区/海上世界下包含
  区内外多个候选的明确选择任意分店；自动候选范围规则、行政区硬冲突、Event 隔离、
  Provider 第一名安全边界、所选 ID 去重/调整/Worker 重放继续由既有测试覆盖。
- `pip check`、Ruff、strict mypy（142 个源文件）通过；用户指定聚焦集
  `175 passed`；非真实 Provider/Map 全集
  `1823 passed, 18 skipped, 2 deselected`；Alembic 唯一 head 为
  `20260729_0017`。未读取 `.env`，真实模型、地图、网页及其他外部 API 调用为 0。
- 主控在候选提交 `222e8fcc4cc564c956baecf87da81ecbac3f5fe6` 上完成真实 Web/H5
  复验：单独选择 M Stand 任意分店，在南山区/海上世界、14:00–18:00 下成功生成
  草案；权威计划快照保存具体 `M Stand（前海壹方汇店）`、任意分店来源和查询时间，
  没有把品牌占位符写入计划。
- 明确选择深圳图书馆和深圳市当代艺术与城市规划馆，在福田区/市民中心、
  14:00–18:00 下成功生成主方案和备选；两个所选收藏均保持可用，排除项为 0。
  本轮真实模型请求为 0；高德按实际生产路径的只读请求上界不超过 5 次，零自动重试，
  没有超过用户授权。
- 两个关联 P1 已关闭，当前无未关闭 P0/P1；M1 仍为已完成，M2-0 尚未开始。
...
- 仅修改收藏页既有 CSS：原生多选 checkbox 直接使用现有 `--touch-target` 作为宽高并
  删除 label 顶部补偿 padding/default margin，实际边界为 `44×44px`。浏览器复验同时
  确认同批新增的“用这些收藏规划”按钮才是原 31.6px 失败值，因此在其既有 selector
  增加同一 44px 最小高度；未修改组件、测试断言、业务逻辑或后端。
- 前端 lint、typecheck、production build 通过；相关 Vitest `72 passed`，收藏页完整
  Playwright `23 passed`。320×568、320×740、390×667、390×844、729×837、
  768×900、1024×900、1440×900 均无横向溢出，所有可见 button、input、select
  均至少 `44×44px`。未读取 `.env`，真实 API 调用为 0。触控尺寸修复完成，等待主控复验。
...
- 基于最终候选 `ca41c0d13f23e7129855cf563d938c73b0356be7`，使用原始 Web/H5
  计划输入和一次自然语言调整完成真实复验：初始上海本地时间为 `14:00–18:00`，
  调整后为 `15:00–18:00` 且节奏为 `relaxed`；两个版本均仅公开
  `has_exact_origin=true`，首段路线已知且没有 `route_unknown`。
- 真实调用为 2 次非流式模型请求、21 次高德只读请求（起点搜索 1、天气 2、路线
  18），自动重试为 0。高德请求超过原授权上限 10 次；用户已明确接受该超限为 QA
  流程问题，不要求重跑。本次收口不再调用任何真实 API，后续真实 QA 必须在脚本层
  强制执行请求总量上限，不能只依赖人工约定。
- 离线主控复验为 `pip check`、Ruff、strict mypy（142 个源文件）、聚焦
  `207 passed`、封网聚焦 `72 passed`、非真实 Provider/Map 全集
  `1807 passed, 18 skipped, 2 deselected`；Alembic 唯一 head 为
  `20260729_0017`。全局地点匹配阈值和收藏行为未改变，没有新增关键词、正则、评分器、
  Provider、重试或 fallback。
- 计划本地时间、调整时间、精确起点、起点主体候选和首段路线 P1 均已关闭；当前无
  未关闭 P0/P1。完整修复链允许纯快进集成到 `main`，M2-0 仍尚未开始。
...
- 修复提交 `376500bffd21e3dc9ea9cccccee24351d78a2c95` 已纯快进集成到
  `main`，合并前后代码树一致，无冲突、merge commit 或额外生产代码变化。
- 使用原始输入“请收藏深圳当代艺术与城市规划馆，地址是福田区福中路184号。”完成
  一次真实 Web/H5 提交、零人工重试；结果直接确认“深圳市当代艺术与城市规划馆”，
  保存正式地址并进入有效收藏，不再被同址附属候选阻塞。修复前的待选择记录仅作为
  既有演示数据保留，不影响新结果。
- 集成门禁通过：`pip check`、Ruff、strict mypy（142 个源文件）、地点与收藏聚焦
  `122 passed`、非真实 Provider/Map 全集 `1793 passed, 18 skipped, 2 deselected`；
  Alembic 唯一 head 为 `20260729_0017`。
- 唯一地点匹配边界、评分阈值、Provider 排序和候选上限保持单一实现；没有关键词
  白名单、第二套 Matcher/Provider/Workflow、迁移、重试或 M2 代码。当前 P1 已关闭，
  无未关闭 P0/P1；M2-0 允许开始但尚未开始。本次集成收口未读取 `.env`，也未追加
  真实 API 调用。

## 2026-08-08 明确主体地点仍被同址附属候选阻塞 P1 修复

- 根因是设施关键词删除后仍保留了候选间名称、地址和辅助字段的“相关非竞争候选”
  判断，既增加了层级推断，也未保证多个 `NAME + ADDRESS + CITY` 强证据候选始终待选择。
- 已删除设施关键词和“相关非竞争候选”判断。唯一 `classify_place_matches` 边界只统计
  同时具有 `NAME + ADDRESS + CITY` 明确 `MATCH` 且无 hard conflict 的强证据候选：
  恰好一个时直接确认并提升为首个安全摘要；多个时始终待选择；没有强证据候选时完整
  沿用既有评分和分差逻辑。评分、阈值、Provider 稳定排序和最多三个候选均未改变。
- 离线回归覆盖固定主体/卫生间/外墙候选且不提供 `source_context`、明确附属对象不反向
  选主体、停车场/入口不互换、地址冲突、同名/连锁/弱证据/Provider rank 安全边界，
  以及收藏一次地图搜索与幂等重放。没有新增 Provider、Matcher、Parser、Workflow、
  状态机、迁移、依赖、fallback、重试、前端或计划改动。`pip check`、Ruff、strict
  mypy（142 个源文件）通过；地点匹配聚焦 `109 passed`，收藏写入 `45 passed`，非真实
  Provider/Map 全集 `1793 passed, 18 skipped, 2 deselected`。
- 修复完成，等待真实用户复验；不据此宣布最终主链路通过。未读取 `.env`，真实模型、
  地图、网页和其他外部 API 调用为 0；全集仅出现既有 aiosqlite 线程收尾 P2 warning，
  M2-0 继续保持未开始。

## 2026-08-08 明确地点错误进入待选择 P1 修复

- 根因位于唯一 `app.domain.places.matching` 评分边界：名称没有把搜索城市的“深圳”与
  “深圳市”视为同一行政前缀；地址只做全文近似，无法识别相同行政区可选前缀、稳定
  道路门牌和供应商括号导航说明；Provider 没有 `branch_name` 时，主体与停车场、入口
  等子设施也缺少结构化分支差异，三个候选因此同分。
- 现有名称、地址和分支证据原位收敛：名称复用既有城市别名，地址仅在双方都有稳定
  道路门牌时按核心严格比较，子设施按通用结构限定词进入既有 `BRANCH_NAME` 证据。
  阈值仍为 unique `75`、minimum gap `12`，Provider rank 仍只负责同分稳定排序。
- 离线回归覆盖场馆主体唯一匹配、明确停车场反向匹配、行政前缀/括号说明、不同道路
  或门牌冲突、连锁多分店、同名跨区、弱证据、Provider 首位不自动确认、一次搜索及
  幂等重放。`pip check`、Ruff、strict mypy（142 个源文件）通过；指定聚焦测试
  `7 passed`，非真实 Provider/Map 全集 `1788 passed, 18 skipped, 2 deselected`。
  生产代码只修改唯一 matcher；没有新增 Provider、Matcher、Parser、Workflow、
  状态机、Repository、迁移、依赖、fallback、重试、前端或计划改动。
- 修复完成，等待真实用户复验；不据此宣布最终主链路通过。未读取 `.env`，真实模型、
  地图、网页和其他外部 API 调用为 0。全集仅出现既有 aiosqlite 线程收尾 P2 warning；
  本修复没有剩余已知 P0/P1，M2-0 继续保持未开始。

## 2026-08-07 Agent 统一意图路由最终集成

- `codex/m1-agent-intent-routing` 以 `--ff-only` 集成到 `main`，集成代码提交为
  `697a5016004a75a29bd97c885228b50fa89dc4c7`；提交链保持线性，合并前后 tree 一致，
  没有冲突、merge commit、squash 或历史改写。
- 普通 Agent 文本继续由唯一 `ModelProvider` 进行语义意图识别；收藏、计划和记忆
  分别复用既有业务服务，计划生命周期继续只有一个领域规则。没有关键词/正则意图
  路由，也没有新增 Provider、Runner、Parser、迁移、重试或 fallback。
- 合并后 `pip check`、Ruff、strict mypy（142 个源文件）通过；计划约束、调整、统一
  意图路由和计划合同共 `85 passed`；Alembic 唯一 head 为 `20260729_0017`。
- Agent 统一意图路由已集成，计划生命周期 P1 已关闭，当前无未关闭 P0/P1。下一步
  仅允许执行 PRD 核心主链路最终真实用户验收；M2-0 尚未开始。
- 本轮未读取 `.env`，真实模型、地图、网页和其他外部 API 调用为 0；未推送 GitHub。

## 2026-08-07 Agent 计划约束生命周期一致性修复

- 根因是直接 `/plans` 入口内联了正确生命周期，而 Agent 路径固定使用两天，计划调整
  继续保留旧值。计划领域现在只有一个纯函数：完整时间窗口使用
  `max(now + 1 小时, end_at + 1 小时)`；时间缺任一端时仅保留一小时用于本轮追问。
  Agent、直接计划入口和调整均复用该边界，调整只延长不足的生命周期，不缩短现有值。
- 五天后 Agent 计划和直接计划均保存至 `end_at + 1 小时`；缺时间仍只追问且不创建
  Plan；计划移到五天后时调整同步延长有效期。一次文本一次模型、收藏、记忆和澄清
  既有合同保持通过。
- `pip check`、Ruff、strict mypy（142 个源文件）通过；用户指定聚焦测试
  `80 passed`，非真实 Provider/Map 全集 `1781 passed, 18 skipped, 2 deselected`。
  全集仅有既有 aiosqlite 线程收尾 P2 warning，本修复无未关闭 P0/P1。
- 未修改 Prompt、意图类型、前端、数据库、迁移、重试或 fallback；未读取 `.env`，
  真实模型、地图、网页 API 调用为 0；未合并、未推送，M2-0 继续保持未开始。
...
- 价格继续复用唯一人民币契约：未知价格不再阻断无预算计划，也不写成 0；明确免费仍为
  `0 + CNY`；硬预算只排除已知超限，未知价格以“预算无法验证”风险进入草案。
- 天气成功、未知和安全 Provider 失败摘要随既有 `PlanVersion` JSON 草案快照保存，旧
  JSON 缺失字段按 `None` 读取；天气失败只形成非阻断风险，不写收藏、Memory 或供应商
  原始响应。无精确起点不再制造 0 秒 / 0 米路线，首段保持未知并提示风险；精确起点的
  路线失败只排除对应候选，后续候选继续。
- 调整继续创建不可变新草案；日历、正式路线和分享统一通过既有 `latest_confirmed()`
  执行边界解析版本。新草案确认前沿用旧确认版，确认后自动切换；既有版本冲突、任务
  终态 CAS、SSE 防旧响应和幂等边界保持不变。前端只展示服务端状态、事实、风险与失败
  原因，并明确区分“已有确认版本，但有新草案”。
- `pip check`、Ruff、strict mypy（141 个源文件）和 Alembic 唯一 head
  `20260729_0017` 通过；后端计划/检索/地图事实/版本/日历/分享聚焦及仓库外
  DNS/socket 封网复跑均为 `155 passed`。非真实 Provider/Map 全集除一个未修改的
  基线固定日期用例外为 `1765 passed, 17 skipped, 2 deselected`；该用例把
  `2026-08-02 18:00 +08:00` 作为计划结束时间，当前执行时已过期，确认审批因既有
  `expires_at > created_at` 约束失败，本修复未用测试特例或产品 fallback 掩盖。
- 前端 lint、typecheck、Vitest `135 passed`，计划与分享 Playwright `5 passed`。
  生产代码净增 208 行；未新增服务、Provider、DTO 类、状态机、规则引擎、数据库字段、
  迁移或依赖，未修改修复 1–3 的收藏、Event、地点或字段失效语义。未读取 `.env`，
  真实模型、高德、网页和其他外部/付费 API 调用为 0；未进入 M2。
...
- 收藏详情确认日期范围时会原子清除历史单端精确场次；完整场次清空任意一端也会同时
  清除两端，完整场次编辑保持既有行为。日期范围仍只显示原生 date 输入。
- 前端不再复制 Event 时间就绪判断，“活动时间待确认”优先采用服务端
...
- 开放期 Event 现在以“完整合法日期范围”或“完整合法固定场次”任一形态确认时间；
  date-range 不再要求或登记缺失的精确场次，部分字段、时间不确定性和冲突仍待确认。
- 已确认 exact PlaceTarget 的 date-range Event 可在计划日期相交时复用既有 visit
  duration 排程；未相交返回 `TIME_WINDOW_CONFLICT`，已结束返回 `EVENT_ENDED`，固定
  场次窗口保持不变。收藏详情只为已有完整固定场次显示原生 time 输入。
- 未新增字段、迁移、依赖、DTO、服务或状态管理；M2-0 仍未开始。指定离线门禁与最终
  提交信息见本分支交接，真实 API 调用为 0，未读取 `.env`。
...
- 主控在候选 `288f0c6a43053dc1e8ca5d42d36bb5bf889f816b` 发现 P1：README
  推荐复制 `.env.example`，但该文件显式设置
  `MODEL_STRUCTURED_OUTPUT_MODE=none`，会覆盖 Settings 与 Compose 的
  `json_object` 默认值，使 API 和 Worker 同时关闭已验真的正式结构化输出。
  定点返修将示例有效值改为 `json_object`，并增加只显式读取仓库
  `.env.example` 的最小 Settings 契约测试，验证原字段值及既有
  `StructuredOutputMode.JSON_OBJECT` 映射；没有修改 Provider、Parser、Extraction
  Workflow 或其他生产代码，也没有增加探测、fallback、重试或第二套配置逻辑。
- 修复 1–3 已通过主控验收；修复 4 已完成定点返修并等待主控复验。M1-Gate
  继续打开，M2-0 未开始且阻塞；不得根据本次开发窗口自行宣布通过。
- 定点复测通过：`pip check`、Ruff、strict mypy（140 个源文件）；配置契约
  `132 passed`；非真实 Provider/Map 全集
  `1725 passed, 16 skipped, 2 deselected`；
  `docker compose --env-file .env.example config --quiet` 通过。安全解析确认 API 与
  Worker 的 `MODEL_STRUCTURED_OUTPUT_MODE` 均为 `json_object` 且一致。未读取
  `.env`，真实模型、高德、网页和付费 API 调用为 0。
- 修复 4 初始候选完成后等待主控验收；M1-Gate 继续打开，M2-0 未开始且阻塞。其
  运行一致性根因是
...
- 主控确认候选 `7a5da03a2de1ccfe8a1c85ce36619dac06d45e21` 直接基于
  `c5608a694c3585ed40d2c8f6079eb4497836e150`，工作区和提交链干净；最终以
  `--ff-only` 集成到 `main`，没有冲突、merge commit 或额外代码变化。
- 独立审查确认仍只有一个收藏详情 modal、一个内部滚动区、一个移动导航和原有
  Event/通用编辑表单；详情打开时底部导航不再绘制或参与焦点顺序，关闭后恢复。
  安全区、sticky 标题、Escape、Tab/Shift+Tab 约束和原卡片焦点恢复均由现有详情
  状态统一处理，没有复制保存、删除、恢复、候选选择或 URL 状态机。
- lint、typecheck、生产构建和完整 Vitest 均通过，Vitest 为 `126 passed`。
  收藏专项 Playwright 为 `21 passed`，完整 Playwright 为 `43 passed`；覆盖
  320×568、320×740、390×667、390×844、729×837、768×900、
  1024×900 和 1440×900。纯快进后的组件复跑为 `44 passed`，收藏专项浏览器
  复跑仍为 `21 passed`。
- Alembic 唯一 head 保持 `20260729_0017`。未读取 `.env`，真实模型、高德、
  网页和其他外部 API 调用为 0；未修改后端、迁移或 M2。
- 当前无未关闭 P0/P1。M1-Gate 继续打开，当前只允许修复 3“Agent 异步提交
  SSE/权威终态恢复”；运行配置修复和 M2-0 继续阻塞。

## 2026-07-31 M1-Gate 修复 2：移动端收藏详情操作可达性

- 修复 2 完成，等待主控验收；M1-Gate 继续打开。SSE 和运行配置修复未开始，
  M2-0 未开始且阻塞。本轮没有修改后端生产代码、迁移或 M2 功能。
- 根因是收藏详情遮罩与固定移动导航共用层级，且导航位于后续 DOM 顺序；详情打开后
  导航仍可接收点击和键盘焦点。详情虽已有滚动，但底部操作没有统一纳入设备安全区。
- 现有详情继续作为唯一 modal 和唯一内部滚动区域，并使用一个语义化 modal 层级；
  打开收藏详情时隐藏现有移动导航，使其不参与绘制、命中测试或焦点顺序，关闭后恢复。
  移动详情底部统一加入 `env(safe-area-inset-bottom)`，标题栏在内部滚动时保持可见，
  没有复制导航、详情、Event 表单、通用编辑表单或保存状态机。
- 关闭按钮保持打开后的初始焦点；Tab/Shift+Tab 被约束在详情内，Escape、关闭按钮
  和删除后的关闭路径均恢复到原收藏卡片。候选选择、“以上都不是”、Event“确认并
  保存”、收藏“保存修改”、删除及恢复继续调用各自原有唯一 API。
- 320×568、320×740、390×667、390×844、729×837 的长 Event 详情均能从标题
  滚动到 Event、通用保存、删除和恢复操作；按钮完整位于视口及安全区域内，中心点
  命中目标，无横向溢出或底层误触。768×900、1024×900、1440×900 的桌面侧栏及
  抽屉行为保持不变。
- 前端依赖安装、lint、typecheck、完整 Vitest、构建、收藏专项及完整 Playwright
  回归均通过；最终数量记录在同日技术验收报告。Alembic 唯一 head 保持
  `20260729_0017`。未读取 `.env`，真实模型、高德、网页和其他外部 API 调用为 0。
...
- P1 返修将正式 POI 合并时的 write-operation 关联收敛集中在现有 Repository
  方法：目标尚未属于当前 operation 时保留原序号并安全改向；目标已经属于当前
  operation 时删除原中间关联，不再写入重复复合主键。应用返回按收藏 ID 保留首次
  顺序去重，回放读取同一份权威关联，Undo 只处理权威收藏一次。全部 Source 继续
  关联权威收藏，中间条目只保留安全逻辑删除状态，不再出现 active 加
  pending_details 的失败部分结果；不同 POI 及不同 operation 指向同一 POI 的语义
  保持不变。
- Agent 的 `pending_selection` 结果卡新增“选择地点”入口，精确指向既有
  `/collections?item=<collection_id>` 收藏详情和候选选择界面；没有复制候选表单或
  选择规则。active、pending_details、deleted 和 Event 时间确认入口未改。
...
- 后端 `pip check`、Ruff、strict mypy（140 个源文件）通过；用户指定聚焦集
  `225 passed`，非真实 Provider/Map 全集
  `1724 passed, 16 skipped, 2 deselected, 1 warning`。仓库外插件封锁 DNS、
  `connect`、`connect_ex` 和 `create_connection` 后，聚焦集仍为 `225 passed`，
  全集仍为 `1724 passed, 16 skipped, 2 deselected, 1 warning`；warning 是既有
  aiosqlite 事件循环关闭后的线程收尾告警，不涉及本轮生产路径，保留为 P2 监测。
- 前端 lint、typecheck 通过；指定组件 `81 passed`，完整 Vitest `124 passed`。
  迁移回归 `25 passed`，Alembic 唯一 head 仍为 `20260729_0017`，无迁移。复杂度
  检查确认未新增 Provider、MatchingService、TargetSelectionService、
  CollectionWriteService、Repository、DTO、状态机或幂等机制，且未修改地点阈值
  和权重；当前无未解决 P0/P1。未读取 `.env`，真实模型、高德、网页和付费 API
  调用为 0；未合并、未推送，地点确认修复 2 未开始。

## 2026-07-30 M1-Gate 修复 4 主控复验与集成

- 主控确认候选 `d0fce9364fb0be2760155a0b1d81819394b2c276` 直接基于
  `4b3b5b80ae0969014e1e52c7f28129ff27bb2a81`，改动仅涉及既有计划应用
  边界、唯一 `PlansExperience`、对应测试和交接文档；没有新增第二套 Provider、
  Runner、计划服务、状态机、SSE Client、幂等服务、Repository、DTO、迁移或 M2
  功能。
- 创建、调整和确认在同一页面会话内的人工重试复用原幂等键，输入、计划版本或
  权威终态改变后才轮换。生成中的权威计划可显式刷新并重新连接既有 SSE；停止
  等待、版本切换和过期响应不会覆盖当前计划。整页刷新继续从服务端计划及版本恢复，
  浏览器不持久化请求密钥。
- 结构化检索失败继续由唯一计划应用边界选择保守共同主因；页面只映射公开错误码，
  不推导新的领域状态、不生成伪成功，也没有为测试样本增加白名单或特例。
- 前端 lint、typecheck、build 通过，Vitest `123 passed`；计划和 Agent 导航
  Chromium E2E 各 `3 passed`。后端 `pip check`、Ruff、strict mypy（140 个源
  文件）通过；计划聚焦集及封网复跑均为 `190 passed`，非真实全集
  `1713 passed, 16 skipped, 2 deselected`，迁移 `25 passed`，Alembic 唯一
  head 为 `20260729_0017`。
- 全集出现 2 条既有 aiosqlite 收尾 warning；目标测试以
  `PytestUnhandledThreadExceptionWarning` 升级为错误后复跑 `1 passed`，未连续
  复现，继续作为 P2 监测。当前无未关闭 P0/P1。未读取 `.env`，真实 API 调用为
  0；修复 4 通过后只允许 M1-Gate 最终复验与收口，M2-0 仍阻塞。
...
- 主控确认最终提交 `b9783e1e7deefc575bbf009df7ff51d0e222069a` 直接位于
  `f85a177fcd76227c9fa9caa06f1df95a186c98fa` 之后，仅以表单级
  `noValidate` 将提交交给既有 `confirmEventTime` 唯一业务校验边界；没有增加
  `onInvalid`、第二套日期规则、校验 helper 或后端旁路。
- 上次两个阻断均已关闭：已有准确场次不会再被有效日期范围端点平移；越界的具体
  场次日期会显示产品中文反馈且 PATCH 为 0。有效日期与准确场次保持独立，原 UTC
  瞬间、只编辑时分、跨夜、单日、多日、清空及失败保留草稿回归均通过。
- 前端 lint、typecheck、build 通过，Vitest `113 passed`，收藏 Chromium E2E
  `14 passed`；后端 `pip check`、Ruff、strict mypy（140 个源文件）通过，收藏
  聚焦回归 `72 passed`、迁移回归 `25 passed`，Alembic 唯一 head 为
  `20260729_0017`。
- 工作区和提交链干净，未读取 `.env`，真实 API 调用为 0；修复 3 当前无未关闭
  P0/P1。M1-Gate 继续打开，当前允许开始修复 4 计划闭环，M2-0 仍未开始且阻塞。

## 2026-07-30 M1-Gate 修复 3 Event 表单校验阻断返修

- M1-Gate 修复 3 表单校验返修完成，等待主控复验；计划闭环与 M2-0 未开始。
  “具体场次日期”继续保留原生 `date` 类型及有效范围 `min/max` 选择提示；仅在
  Event 时间表单设置 `noValidate`，使越界提交进入既有 `confirmEventTime` 唯一
  业务校验边界，展示中文范围反馈且 PATCH 为 0。
- 没有增加 `onInvalid`、校验 helper 或第二份日期规则。日期倒置、缺少场次日期、
  范围越界、结束不晚于开始、清空时间及失败保留草稿均继续走原有路径；有效日期与
  准确场次保持独立，已有准确瞬间、时分编辑日期和跨夜日期语义未改动。
- 前端 lint、typecheck、build 通过，Vitest `113 passed`；收藏 Chromium E2E
  `14 passed`，新增真实表单提交回归证明浏览器原生约束不再截断产品反馈。后端
  `pip check`、Ruff、strict mypy（140 个源文件）通过，指定聚焦回归
  `72 passed`、迁移回归 `25 passed`；Alembic 唯一 head 为 `20260729_0017`。
- 本轮未修改后端、DTO、数据库、迁移、Prompt、地点匹配、计划功能或 M2；未读取
  `.env`，真实 API 调用为 0；未合并、未推送。

## 2026-07-30 M1-Gate 修复 3 Event 时间语义 P1 返修

- M1-Gate 修复 3 返修完成，等待主控复验；计划闭环与 M2-0 未开始。
  候选 `f89a9531d57c7fbe66cd1f74573dc2c3658c89da` 将准确开始/结束时间分别
  拼到有效范围端点，可能把多日活动内的一日场次静默延长到有效结束日；该行为已
  被替代。活动有效自然日范围和准确场次时间现作为两类独立事实处理。
- 收藏详情继续使用两个有效日期 `date` 和两个具体时间 `time` 控件。已有准确场次
  分别保留原开始、结束瞬间的上海自然日；未编辑直接确认原值完全不变，只编辑
  `HH:mm` 时仅替换时分并保留各自原日期。修改有效范围不会平移准确场次；若新范围
  排除了既有场次，则明确提示并拒绝 PATCH。
- 没有准确场次时，单日有效范围可直接组合该自然日与时间；多日范围不再猜测场次
  日期，而显示唯一“具体场次日期”控件。该日期必须位于有效范围内，未填或越界均
  不发送 PATCH；开始和结束时间使用同一个已确认场次日期。只保存日期仍保持准确
  时间为 `null` 和“具体时段待补充”，清空具体时间继续显式提交 `null`。
- 日期倒置、缺少必要日期及组合后结束不晚于开始仍在请求前拒绝。409、422、超时、
  取消和网络错误保留有效日期、场次日期和两个时间草稿。规划资格、地点阻塞和保存
  结果继续只读服务端响应，`location_unconfirmed` 不由前端重新推导或覆盖。
- 前端 lint、typecheck、build 通过；Vitest `113 passed`；收藏 Chromium E2E
  `13 passed`，320、390、768、1024、1440px 无横向溢出，字段标签和聚焦能力
  通过。后端 Ruff、strict mypy（140 个源文件）通过，指定三组最小契约回归
  `52 passed`；Alembic 唯一 head 为 `20260729_0017`。
- 本轮只修改收藏详情及其前端测试和状态文档。继续复用原收藏 PATCH 与原有两个
  上海时间转换函数；未新增 DTO、Client、服务、状态机、Repository、后端代码、
  数据库或迁移，也未修改地点匹配、Prompt、计划闭环或 M2。未读取 `.env`，真实
  API 调用为 0；未合并、未推送。
...
- M0-5C 已完成：历史确定性草案入口、交通与结束缓冲、费用/来源/风险、任意分店快照、20 组零硬约束违反 Fixture，以及中国场景下严格的 `None + None` 或 `Decimal + CNY` 价格契约均已通过主控验收；其候选组合产品规则已被当前模型提案定义取代。
...
- M0 关闭后 Event 日期粒度修正已完成：有效自然日期与准确场次时间使用独立字段，
  完整合法日期范围足以确认 Event 时间，并可在计划日期相交时按既有 visit duration
  排程；部分字段、不确定时间和无效范围仍待确认。没有样本白名单或第二套解析链。
...
- 生成行为：M0-5C 历史草案实现验证了 pace 对交通缓冲和结束留白的确定性映射；缺少候选访问时长、首段路线、Event 固定时间、任意分店查询时间，或无法形成任何可执行组合时不猜测并返回稳定不可生成原因；历史候选组合规则不再是当前产品定义。
...
- 验收结论：唯一 `PlanDraftService` 在 M0-5A/M0-5B 已验收契约上复核时间、Event、路线、交通方式、缓冲、结束留白、预算、费用、来源、风险、任意分店快照和重复 POI。20 组固定 Fixture 的硬约束违反数为 0；当前产品定义将候选组合、顺序和建议停留时长交由模型提案。
...
- 状态与计划：2A 已替代早期 date-only 限制；当前完整合法日期范围或完整合法精确场次
  任一足以确认 Event 时间，日期范围与计划日期相交时按既有 visit duration 排程
...
- 纯度与范围：真实模型、地图、网页、对象存储、消息及其他外部 API 调用为 0；
  ...
  没有生产特例
- 下一步（历史）：等待主控复核迁移安全、严格字段闭合、日期/时刻区分和 exact-time
  回归；date-only 计划准入当前以 2A 规则为准。M0 保持完成，M1-0 仍未开始；其第一项
  强制前置仍是 `IdempotencyLockRegistry` 有界生命周期修复
...
#### 2026-08-03｜M1 PRD 核心主链路真实用户复验与收敛｜已完成

- 授权与范围：只按 PRD 核心主链路使用运行中的完整 Web/H5、真实模型和高德；
  模型与只读高德请求上限各 20 次，零自动重试，不测试偏离产品目标的输入。按可证实
  上界记录为模型不超过 6 次、高德不超过 10 次，均未越过授权；未输出或提交密钥、
  完整请求、响应、分享 token、原始截图或私人路径
- 真实闭环：文字收藏、截图与文字共同提交、URL 收藏、地点候选确认、Event 日期与
  精确 POI 确认、计划生成、自然语言调整、版本确认、路线/日历入口、只读分享、完成
  反馈及 Memory 建议确认均成功；匿名分享保持只读，截图在输入文字后不再消失
- 根因收敛：候选确认后编辑草稿改为读取同一权威收藏；Agent SSE 观察连接只增加
  30 秒前端观察看门狗并回到既有权威结果读取；Event 地图查询优先使用已确认地标，
  不确定地标仍使用地址；未知营业状态、天气和路线按既有风险进入草案而非错误阻断；
  新旧分享快照统一复用一个中文风险标签映射；收藏文案不再把基础就绪误报成当前
  时间/范围一定可用。没有增加第二套 Provider、Runner、工作流、解析器或状态机
- 自动化复验：后端 pip check、Ruff、strict mypy（141 个源文件）通过，非真实全集
  `1771 passed, 18 skipped, 2 deselected`；前端 lint、typecheck、生产 build 通过，
  Vitest `140 passed`，Playwright `46 passed`，覆盖 320/390px、长 Event 详情、
  Agent 长任务恢复、收藏、计划、Memory 与匿名分享
- 结论：本轮发现的 P1 均已由共享根因修复并完成真实与离线复验，当前无未关闭
  P0/P1。M1 仍为已完成，当前允许阶段仍为 M2-0；本轮没有实现任何 M2 功能
