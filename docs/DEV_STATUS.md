...
| 当前子阶段 | M1-8 只读分享能力 |
| 状态 | 待主控验收 |
| 当前分支 | codex/m1-8-readonly-sharing |
| 最近更新 | 2026-07-29 |
| 阻塞项 | 开发无阻塞；本机无 PostgreSQL 运行时或 TEST_POSTGRESQL_URL，真实 PostgreSQL 并发测试待主控环境复跑 |
...
阶段不得提前开发。
M1-7 已通过主控最终验收并纯快进集成：结构化 Memory、可审计偏好候选、当前请求
优先的 pace 默认、城市无关粗区域、有效检索与使用记录、M07 控制页及私有导出均已
复核。当前无未关闭 P0/P1；M1-8 只读分享能力是唯一允许开始的阶段。
M1-8 开发已完成并等待主控验收：行程级 256-bit 哈希分享、最新确认版本脱敏快照、
即时撤销/重建、行程结束七天过期、取消与统一无内容状态、匿名 GET 边界以及 M08
Web/H5 页面均已实现。后续阶段仍不得提前开发。
...
| M1-8 只读分享能力 | 待主控验收 | 哈希 bearer、最新确认脱敏快照、撤销/重建/七天过期、匿名 GET 与 M08 响应式页面已完成；真实 PostgreSQL 复跑待主控环境 |
...
主控在具备 PostgreSQL 的隔离环境复跑 M1-8 行锁并发、迁移往返与完整 QA；通过后
只允许纯快进集成本阶段提交。不得提前实现微信、主动提醒、云部署或后续阶段。
