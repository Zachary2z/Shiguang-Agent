...
| 当前子阶段 | M1-5 计划生成、调整和确认 |
| 状态 | M1-4 已完成；M1-5 未开始 |
| 当前分支 | main |
| 最近更新 | 2026-07-28 |
| 阻塞项 | 无；当前允许开始 M1-5，M1-6 及后续阶段不得提前开始 |
...
当前唯一允许阶段为 M1-5 计划生成、调整和确认；M1-6 及后续阶段不得提前开发。
...
从最新 `main` 创建 `codex/m1-5-plan-experience`，只实施 M1-5 计划生成、调整和
确认：时间与活动范围、可选预算、条件确认、后台进度、时间光轨、主方案和备选、
费用/交通/来源/风险、外部补充授权、自然语言调整、版本隔离和明确确认。必须复用
现有 PlanConstraints、计划草案、收藏检索、Approval、Job/SSE、Session 和 API
Client，不建立第二套规划器或前端状态框架；不得提前实现 M1-6 日历、路线执行与
反馈，也不得读取真实密钥或调用未授权外部 API。
