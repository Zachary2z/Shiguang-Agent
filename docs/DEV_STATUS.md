...
| 当前子阶段 | M1-4 收藏库与地点消歧 |
| 状态 | M1-4 待主控验收（P1 修复完成，待主控复验） |
| 当前分支 | codex/m1-4-collections-disambiguation |
| 最近更新 | 2026-07-27 |
| 阻塞项 | 无；M1-4 P1 修复完成，待主控复验，M1-5 不得提前开始 |
...
| M1-4 收藏库与地点消歧 | 待主控验收 | P1 修复完成，待主控复验；详情操作归属、合并后 URL 与全来源迁移已补齐 |
...
重点检查详情写操作归属、候选合并后的 URL/刷新一致性、exact/any-branch 全来源
迁移与幂等/用户隔离，并回归组合筛选、规划隔离、版本冲突、响应式与键盘闭环。
验收前不合并、不推送，不开始 M1-5 计划或后续业务。
