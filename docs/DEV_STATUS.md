...
- 根因是上一版在分支证据中枚举停车场、车库和出入口，无法覆盖卫生间、外墙等其他
  附属对象；主体虽有明确名称和地址，仍被同址名称包含候选按普通竞争者计算分差。
- 已删除设施关键词规则。唯一 `classify_place_matches` 边界现在复用既有名称关系、名称
  行政前缀归一化、地址道路门牌归一化和结构化 evidence：仅当首位名称、地址均明确
  `MATCH`，同址候选名称只是包含关系且没有额外明确证据时，该候选不阻塞唯一性；候选
  仍保留在安全摘要中。评分、阈值、Provider 顺序和最多三个候选均未改变。
- 离线回归覆盖固定主体/卫生间/外墙候选且不提供 `source_context`、明确附属对象不反向
  ...
  mypy（142 个源文件）通过；地点匹配聚焦 `107 passed`，收藏写入 `45 passed`，非真实
  Provider/Map 全集 `1791 passed, 18 skipped, 2 deselected`。
