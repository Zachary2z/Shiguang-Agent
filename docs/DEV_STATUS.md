...
| 当前分支 | codex/m1-gate |
| 最近更新 | 2026-07-29 |
| 阻塞项 | 无 P0/P1；真实性能等待当前任务明确授权，M1-Gate 待主控最终验收 |
...
| M1-Gate 核心闭环验收 | 进行中 | 离线、封网、PostgreSQL 16、Compose、前端、21 组闭环和性能观测通过；真实性能等待授权，尚未关闭 M1 |
...
`docs/technical/M1_VALIDATION_REPORT.md`。如需验证真实文本、地图、网页或图片
性能，必须在当前任务明确限定样本数、最大请求数和费用后另行授权；授权前真实调用
保持 0，M1-Gate 不得标记完成。不得提前实现微信、主动提醒、云部署或 M2。
