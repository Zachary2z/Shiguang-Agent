...
- 本轮离线复验：`pip check`、Ruff 和 strict mypy（140 个源文件）退出码均为 0；
  后端全量 `1684 passed, 18 skipped`，仓库外插件封锁 DNS/socket 后的语义、统一
  输入和内容导入聚焦集 `136 passed`；前端单测 `87 passed`、生产构建和浏览器
  E2E `29 passed`，覆盖
  320/390/768/1024/1440 宽度；Alembic 唯一 head 仍为 `20260729_0017`。
- 当前没有本次修复候选中已知未关闭的 P0/P1；既有 P2 仍为 npm 开发依赖 audit
  和跨浏览器覆盖限制。状态只到“待主控验收”，M1 未重新关闭。
