...
| 最近更新 | 2026-07-27 |
| 阻塞项 | 无；M1-4 已完成开发并等待主控验收，M1-5 不得提前开始 |
...
恢复与继续添加。M1-4 收藏库与地点消歧已完成开发并进入待主控验收；M1-5 及后续
阶段不得提前开发。
...
| M1-4 收藏库与地点消歧 | 待主控验收 | 用户隔离列表/详情、统一筛选分页、城市分组、候选选择、删除恢复和正式响应式页面已完成 |
...
主控从 `codex/m1-4-collections-disambiguation` 的阶段提交独立复测 M1-4：
重点检查组合筛选与稳定分页、城市待确认/其他城市规划隔离、候选选择语义、
expected_version 冲突、URL 状态恢复、响应式与键盘闭环。验收前不合并、不推送，
不开始 M1-5 计划或后续业务。
