...
- 安全与副作用：`origin` 不进入 repr 或公开序列化；活动范围和 include/exclude 不进入 repr。`PlanContract` 的唯一安全错误边界会重建 Python/JSON 校验失败，移除原输入与异常链、固定错误项 `input=None`、掩码敏感位置并限制消息；`str`、`repr`、`errors()`、`json()` 和普通异常日志均不依赖 `hide_input_in_errors` 单独脱敏。解析函数不导入或调用 Repository、数据库、Provider、模型、地图、路线、天气、文件或网络，不修改输入；本阶段没有 API、Worker、队列、前端、缓存、Prompt、Tool Calling 或 `nanobot_core` 改动
...
- 阻断根因与唯一边界：原实现只依赖 `hide_input_in_errors=True`，因此 `str(ValidationError)` 已隐藏输入，但 `errors()` 与 `json()` 仍持有跨字段校验收到的完整模型输入。修复继续使用唯一 `PlanContract` 和标准 Pydantic `ValidationError`，由一处共享重建函数清除每项原始 input/context、掩码敏感或未知位置、仅保留固定白名单消息，并在异常捕获块之外抛出以丢弃原异常链；Python 构造/`model_validate` 由完整 core schema 边界覆盖，`model_validate_json` 在保留 Pydantic 原生 JSON 严格解析后进入同一重建边界，没有新增第二套错误、Coordinate、PlanConstraints 或公共校验契约
...
- 真实调用与主控复测：未读取、打印或修改本机 `.env`，真实模型、高德、路线、天气、网页、DNS、对象存储、消息及其他外部/付费 API 调用均为 0。当前没有已知未关闭 P0/P1；主控应重点复测直接构造、Python/JSON model validation 的结构化错误和异常日志、成功序列化、严格输入语义及完整离线回归。状态仍为 M0-5A 待主控验收，M0-5B/C/D 保持未开始，本分支不合并、不推送
