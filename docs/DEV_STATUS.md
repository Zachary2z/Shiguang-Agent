...
响应隔离均已复核。当前允许开始 M1-7；M1-8 及后续阶段不得提前开发。
...
| M1-7 我的、记忆和数据控制 | 未开始 | 当前唯一允许阶段；不得提前实现 M1-8 分享或后续能力 |
...
从最新 `main` 创建 `codex/m1-7-memory-data-control`，只实现 M1-7“我的、记忆和
数据控制”：查看长期记忆及来源/确认状态、确认推断偏好、修改/删除、临时约束过期、
数据导出基础，以及对尚未实现提醒能力的真实关闭状态展示。必须复用现有计划产生的
待确认偏好建议和唯一用户/Session 边界，不建立第二套记忆、偏好或导出状态。M1-8
只读分享及后续阶段不得提前开发；默认测试继续完全离线。
