...
| 当前分支 | codex/m1-gate |
| 最近更新 | 2026-07-29 |
| 阻塞项 | P1：两个真实地图样本稳定返回 INVALID_RESPONSE，待安全结构分类归因；M1-Gate 待主控最终验收 |
...
只读边界、标准地图入口和 M08 Web/H5 均已复核。当前无未关闭 P0/P1，唯一允许
开始的阶段为 M1-Gate。
...
| M1-Gate 核心闭环验收 | 进行中 | 离线、封网、PostgreSQL 16、Compose、前端、21 组闭环通过；地图定点诊断排除鉴权/限流/网络，两样本仍为 INVALID_RESPONSE |
...
`MAP_PROVIDER_INVALID_RESPONSE`。现有授权边界内无法确定具体结构类别，尚未证明
生产代码缺陷；如继续诊断，必须重新授权不保存字段值的安全结构分类。阻塞关闭前
M1-Gate 不得标记完成。不得提前实现微信、主动提醒、云部署或 M2。
