...
- 当前 MVP 和既定 M0–M3 路线继续只支持深圳，不在 M0-2B 或后续既定阶段夹带城市切换实现。
- 深圳单城市核心闭环稳定后，可独立启动 `U1 单活动城市切换`：用户在已开放城市间显式切换，但一次会话、抽取和计划仍只使用一个活动城市；跨城多日规划继续不在范围内。
- 当前无需增加空接口、城市表或兼容层；新代码只需避免进程级可变城市状态，并继续让地图和计划契约显式携带城市参数。
- 正式启动 U1 前必须先补 ADR，确认城市目录、Session 切换语义、数据迁移、高德城市编码和各城市测试 Fixture。
