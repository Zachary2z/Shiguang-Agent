...
- 安全与副作用：`origin` 不进入 repr 或公开序列化；活动范围和 include/exclude 不进入 repr。`PlanContract` 在编译完成的 Pydantic core validator 外安装唯一安全边界，直接构造、BaseModel 与 TypeAdapter 的 Python/JSON/malformed JSON 路径均重建失败，移除原输入与异常链、固定错误项 `input=None`、掩码敏感位置并限制消息；`str`、`repr`、`errors()`、`json()` 和普通异常日志均不依赖 `hide_input_in_errors` 单独脱敏。解析函数不导入或调用 Repository、数据库、Provider、模型、地图、路线、天气、文件或网络，不修改输入；本阶段没有 API、Worker、队列、前端、缓存、Prompt、Tool Calling 或 `nanobot_core` 改动
...
- 迁移、范围与下一步：未新增或修改迁移、ORM、Repository、API、Provider、数据库、依赖、配置、前端、M0-5B 检索、M0-5C 草案或 M0-5D 外部补充；Alembic head 保持唯一 `20260722_0006`。未读取、打印或修改 `.env`，真实模型、高德、路线、天气、网页、对象存储、消息及外部/付费 API 调用均为 0。当前无已知未关闭 P0/P1；状态保持 M0-5A 待主控验收，不合并、不推送、不开始 M0-5B/C/D
