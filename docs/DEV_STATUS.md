...
响应隔离均已复核。M1-7 开发已完成并待主控验收：结构化 Memory、M1-6 建议
确认/拒绝、有效记忆计划检索、实际使用记录、M07 管理页和私有数据导出均已实现。
M1-8 及后续阶段不得提前开发。
...
| M1-7 我的、记忆和数据控制 | 待主控验收 | 结构化 Memory、授权决定、有效检索、使用记录、M07 控制页和私有导出已完成；不得提前实现 M1-8 |
...
主控复核 `codex/m1-7-memory-data-control` 的线性迁移、Memory 授权/所有权、计划
检索与使用记录、M07 竞态隔离和私有导出。验收通过前保持“待主控验收”，不合并、
不推送、不开始 M1-8。M1-8 只读分享及后续阶段仍不得提前开发。
