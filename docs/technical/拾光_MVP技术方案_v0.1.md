...
- 完整合法日期范围或完整合法精确场次任一足以确认 Event 时间；部分字段、无效顺序、时间冲突或相关 uncertainty 仍保持待确认；
- 日期范围 Event 仅在计划日期与范围相交时进入候选；没有精确场次时，在用户空闲窗口内复用既有 visit duration 排程；
...
6. 主要依赖外部地点时先创建授权；
7. 唯一 `ModelProvider` 一次返回三个提案，决定候选组合、顺序、建议停留时长和简短理由；
8. 确定性服务根据事实计算准确时间、交通、费用和缓冲；
9. 确定性校验时间窗、Event、预算、路线和候选来源等硬约束；
10. 校验后保存合法草案，不用规则改写模型的候选选择。

### 12.3 候选选择边界

第一版不新增候选评分系统。收藏服务只执行事实和硬约束过滤；模型只能引用本次提供的 opaque `candidate_key`，用户多选项默认为优先项，明确标记的必须项不得静默遗漏。

### 12.4 草案约束

- 初次生成恰好一个主方案和两个备选，备选不预设固定风格；
- 每个方案的候选数量和建议停留时长由模型提出，不固定 60 分钟或两个地点；
- 交通切换预留 10–20 分钟；
- 结束前保留 15–30 分钟；
- 不为填满时间而自动外搜；
...
11. 完成真实延迟基准和超时校准，能够用单次调用与端到端数据说明当前时限是否足够。

### 20.4 真实延迟与超时校准

- M0-Gate 分开测量文本单次抽取、结构修复、模型 → Tool → 模型、高德、普通/重定向网页和图片识别；
- 每类结果记录样本数、P50、P95、最大耗时、超时率、重试率、恢复结果、Token 和费用，同时区分外部调用耗时与用户端到端耗时；
