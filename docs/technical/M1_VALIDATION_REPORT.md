...
**结论：通过。** 修复提交
`376500bffd21e3dc9ea9cccccee24351d78a2c95` 已纯快进集成到 `main`。
原始输入“请收藏深圳当代艺术与城市规划馆，地址是福田区福中路184号。”经一次真实
Web/H5 提交、零人工重试，直接确认正式主体 POI 并进入有效收藏，不再被同址卫生间、
外墙等附属候选阻塞。

- 最终规则只在唯一 `classify_place_matches` 边界判断强证据：名称、城市和道路门牌均
  明确匹配且无硬冲突的候选恰好一个时确认；多个时待选择；没有时沿用既有评分与分差。
- 没有设施关键词白名单、候选层级代理、第二套 Matcher/Provider/Workflow、阈值放宽、
  迁移、重试或 M2 实现。
- `pip check`、Ruff、strict mypy（142 个源文件）通过；地点与收藏聚焦
  `122 passed`；非真实 Provider/Map 全集 `1793 passed, 18 skipped, 2 deselected`；
  Alembic 唯一 head 为 `20260729_0017`。
- 当前 P1 已关闭，无未关闭 P0/P1。M2-0 允许开始但尚未开始；集成收口未读取 `.env`，
  未追加真实 API 调用。

## 2026-08-02 M1 稳定化修复 1：最终主控复验

**结论：通过并纯快进集成。** 返修提交
`6f26be9497411619edffd6be4fb00aeaa7deb39d` 线性追加在初始候选
`7fa9df5468e536e2a333ed69a516bcb5229f4715` 之后，完整修复链直接基于
`e1e9e5a32c20f711cc65836a1706c2d80b62358c`，无 merge commit 或额外代码变化。

- 主控复核确认地点身份编辑只有一次权威写入：匹配失败、取消和 CAS 冲突均保持原
  收藏全部字段、状态、target、候选快照和版本不变；匹配成功才由既有选择服务一次
  提交编辑与解析结果。旧补偿提交路径已删除，没有第二套状态机或事务协调器。
- 官方高德 identity 查询只发一次详情请求并从同一响应确定支持城市；深圳和广州复用
  唯一 POI 映射与硬事实校验，未知城市和响应事实冲突分别安全映射，未回退深圳、未
  多城市探测、未新增 Provider、HTTP Client 或重试。
- 独立验证通过：`pip check`、Ruff、strict mypy（141 个源文件），Alembic 唯一 head
  `20260729_0017`；聚焦后端 `376 passed`；非真实 Provider/Map 全集
  `1752 passed, 16 skipped, 2 deselected`；前端 lint、typecheck 和 Vitest
  `135 passed`。仓库外插件封锁 DNS 和 socket 连接后，聚焦集 `287 passed`。
- 封网聚焦顺序出现一次既有 `aiosqlite` 线程收尾 warning；将其升级为错误后，在未含
  本次修复的指定 main 基线也可复现，因此维持已登记 P2，不作为本次地点修复回归。
  正式非真实全集无 warning，功能与数据断言全部通过。
- 本轮真实 API 调用为 0，未读取 `.env`；当前无未关闭 P0/P1。M1 关闭结论不变，
  M2-0 仍只允许开始且尚未开始。
...
## 11. PRD 核心主链路真实用户复验（2026-08-03）

用户授权只覆盖 PRD 核心主链路，模型与只读高德请求各不超过 20 次、零自动重试。
本轮按可证实上界记录为模型不超过 6 次、高德不超过 10 次；没有测试偏离产品目标的
输入，也没有记录密钥、完整请求/响应、分享 token、原始截图或私人路径。

真实 Web/H5 依次完成：文字、截图加说明文字、URL 收藏；地点候选与 Event 时间/POI
确认；计划生成、自然语言调整和版本确认；路线、日历、只读分享；完成反馈及 Memory
建议确认。过程中发现并关闭以下共同根因：

- 候选确认后本地编辑草稿没有从权威收藏恢复，导致后续无关修改覆盖 POI；
- SSE 观察连接可能不结束，页面缺少回到既有权威结果读取的观察截止；
- Event 用描述性地址而不是已确认地标做地点匹配；
- 未知营业状态被误作计划硬阻断；
- 旧分享快照的英文风险摘要没有经过唯一显示映射；
- 收藏结构就绪文案误导为当前时间和范围必然可参与。

修复均位于现有共享边界，没有新增 Provider、Runner、工作流、解析器、repair、计划
规则副本或前端状态机。最终后端非真实全集为
`1771 passed, 18 skipped, 2 deselected`，pip check、Ruff、strict mypy（141 个源
文件）通过；前端 Vitest `140 passed`、Playwright `46 passed`，lint、typecheck 与
生产 build 通过。Playwright 覆盖 320/390px 无横向溢出、长 Event 详情可达、Agent
长任务恢复、收藏、计划、Memory 和匿名分享。

当前没有未关闭 P0/P1。跨浏览器/真实设备日历与地图拉起、真实性能大样本仍按既有
P2/风险管理，不阻塞已完成的 M1；当前允许阶段仍为 M2-0。
