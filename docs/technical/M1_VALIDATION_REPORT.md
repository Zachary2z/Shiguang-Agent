...
本轮只收敛收藏详情的 Event 确认交互和必要的前端时间转换测试：

1. 活动有效开始/结束日期继续使用两个 `date` 控件；具体开始/结束时间改为两个
   `time` 控件，只显示和编辑 `HH:mm`，不再重复显示年月日。
2. 现有两个转换函数原位收敛：服务端 ISO 瞬间固定按 `Asia/Shanghai` 提取日期和
   `HH:mm`；提交时将开始日期与开始时间、结束日期与结束时间分别组合成带
   `+08:00` 的 ISO 8601。没有新增第二套日期/时间 helper，也不使用浏览器所在
   时区或当前日期/时间。
3. 日期字段为空但已有准确时间时，只从对应准确时间的上海自然日恢复页面草稿；
   打开详情不会自动写库。单日活动允许两端日期相同；多日活动使用各自日期。
4. 只确认日期可以保存，`event_start_at/event_end_at` 保持 `null`；清空已有具体
   时间显式通过原 PATCH 提交 `null`。只填写一个具体时间可以保存，但服务端仍返回
   待补充，前端不会显示可规划成功。
5. 请求前拒绝日期倒置、具体时间缺少对应日期及组合后结束不晚于开始；同日和跨日
   均比较完整的上海组合时刻。规划资格、地点阻塞和保存结果继续只采用服务端
   `planning_eligible/planning_exclusion_reason`，未复制 Event 状态机、规划规则
   或“公开地址即正式地点”的判断。
6. 409、422、超时、取消和网络错误保留日期与时间草稿；服务端返回地点未确认时，
   即使时间完整仍展示地点阻塞。
...
- `npx playwright test tests/e2e/collections.spec.ts`：`13 passed`，Event
  表单在 320、390、768、1024、1440px 无横向溢出，原生标签和键盘顺序通过；
- 后端 Ruff、strict mypy（140 个源文件）：通过；
- `test_m0_2d_api.py`、`test_collection_write_service.py`、
  `test_m1_4_collections.py`：`52 passed`；
- Alembic 唯一 head：`20260729_0017`。

本轮没有后端代码、迁移、DTO、API、Repository、确认服务、Prompt、抽取规则、
地点匹配、计划生成/调整/确认、Agent 输入或 M2 变更。未读取 `.env`，真实模型、
地图、网页和付费 API 调用为 0；未合并、未推送。
