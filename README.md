...
主控验收。其后真实运行复验重新打开 M1-Gate。** 当前
`codex/m1-stabilization-core-loop` 的稳定化修复候选处于“待主控验收”：
M1 尚未重新关闭，M2-0 未开始并阻塞于主控复验。
...
删除截图；“重试”会用同一份已准备输入发起一次新的提交。收藏补充成功后会直接进入
既有地点候选选择，保存失败则恢复服务端权威数据；“帮我安排时间”进入既有计划页。
