...
`PlanContract` 在编译完成的 Pydantic core validator 外提供唯一安全边界；直接构造、`model_validate*` 和 `TypeAdapter.validate_*` 会复用同一 validator，因此 Python、JSON 和 malformed JSON 错误都在离开契约前统一重建。该边界直接委托原生 `validate_python`/`validate_json`，不会改变严格 Python 输入，也保留 JSON 的 ISO datetime、数字到 Decimal 和字符串枚举解析；不再维护单独的 `model_validate_json` 脱敏 override。安全错误的 `input` 固定为 `None`，context 仅含固定安全消息，敏感或未知位置被掩码，原异常链被丢弃。因此 `str`、`repr`、`errors()`、`json()` 和普通异常日志都不会携带精确 `origin`、私人活动范围、include/exclude 私人要求或原始完整输入；这项保证不只依赖 `hide_input_in_errors`。成功对象的精确 `origin` 仍不进入 repr、日志、`model_dump()` 或 `model_dump_json()`。聚焦回归入口为 `APP_ENV=test RUN_REAL_MODEL_TESTS=0 RUN_REAL_MAP_TESTS=0 python -m pytest -q tests/unit/test_plan_constraints.py`。
