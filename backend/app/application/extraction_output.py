...
def canonicalize_extraction_result(
    ...
    *,
    ...
    if result.outcome is ExtractionOutcome.CANDIDATES:
        return ExtractionResult.with_candidates(
            tuple(_mark_event_time_suggestions(candidate) for candidate in result.candidates)
        )
...
def _mark_event_time_suggestions(
    candidate: PlaceCandidate | EventCandidate,
) -> PlaceCandidate | EventCandidate:
    """Keep model-proposed Event times visible but unconfirmed across every input."""

    if not isinstance(candidate, EventCandidate):
        return candidate
    existing = {uncertainty.field for uncertainty in candidate.uncertainties}
    uncertainties = list(candidate.uncertainties)
    for field in EVENT_TEMPORAL_FIELDS:
        if getattr(candidate, field.value) is not None and field not in existing:
            uncertainties.append(
                Uncertainty(
                    field=field,
                    reason="模型识别的活动时间需要用户确认。",
                )
            )
    if len(uncertainties) == len(candidate.uncertainties):
        return candidate
    values = candidate.model_dump(mode="python")
    values["uncertainties"] = tuple(uncertainties)
    return EventCandidate.model_validate(values)
